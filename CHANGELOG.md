...
- You should review the [release notes for v1.0.2](https://github.com/GetGoji/goji-blockchain/releases/tag/1.0.2) but we especially want to point out that wallet sync is much faster than in 1.0.1 and earlier versions.
...
- `goji farm summary` will now display TXGJ or XGJ as appropriate.
...
- Some users can't plot in the GUI in MacOS Big Sur - especially on M1. See issue [1189](https://github.com/GetGoji/goji-blockchain/issues/1189)
...
- RC3 is a new chain to support the last major gojilisp changes. TXGJ from the RC1/2 chain do not come forward to this chain but plots and keys continue to work as usual.
...
- The entire CLI has been ported to [Click](https://click.palletsprojects.com/en/7.x/). Huge thanks to @jespino for the big assist and @unparalleled-js for the [recommendation and the initial start](https://github.com/GetGoji/goji-blockchain/issues/464). This will make building out the CLI much easier. There are some subtle changes and some shortcuts are not there anymore. `goji -h` and `goji SUBCOMMAND -h` can be your guide.
...
- This is an errata release for Release Candidate 1. There were a couple of things that did not smoothly migrate from the Beta versions. Please make sure you also consult the [release notes for RC-1](https://github.com/GetGoji/goji-blockchain/releases/tag/1.0rc1) was well.
...
- There is now remote UI support. [Documents](https://github.com/GetGoji/goji-blockchain-gui/blob/main/remote.md) will temporarily live in the repository but have moved to the [wiki](https://github.com/GetGoji/goji-blockchain/wiki/Connecting-the-UI-to-a-remote-daemon). Thanks to @dkackman for this excellent addition!
...
- 'goji wallet send' now takes in TXGJ or XGJ as units instead of mojis.
...
- The GUI is now in a separate submodule repository from goji-blockchain, [goji-blockchain-gui](https://github.com/GetGoji/goji-blockchain-gui). The installers and install scripts have been updated and it continues to follow the same install steps. Note that the GUI directory will now be `goji-blockchain-gui`. The workflow for this may be "touch and go" for people who use the git install methods over the short term.
...
- The cli wallet, `goji wallet`, was incorrectly displaying TXGJ balances as if they were Coloured Coins.
...
- Significant improvements have been made to how the full node handles the mempool. This generally cuts CPU usage of node by 2x or more. Part of this increase is that we have temporarily limited the size of transactions. If you want to test sending a transaction you should keep the value of your transaction below 20 TXGJ as new consensus will cause you to use a lot of inputs. This will be returned to the expected level as soon as the integration of [clvm rust](https://github.com/GetGoji/clvm_rs) is complete.
...
- We have made the "Help Translate" button in the GUI open in your default web browser and added instructions for adding new translations and more phrases in existing translations at that [URL](https://github.com/GetGoji/goji-blockchain/tree/main/electron-react/src/locales). Try the "Help Translate" option on the language selection pull down to the left of the dark/light mode selection at the top right of the GUI.
...
- We are down to only one sub dependency that does not ship binary wheels for all four platforms. The only platform still impacted is ARM64 (generally Raspberry Pi) but that only means that you still need the minor build tools as outlined on the [wiki](https://github.com/GetGoji/goji-blockchain/wiki/Raspberry-Pi).
...
- Last Attempted Proof is now fixed. This will show you the last time one of your plots passed the [plot filter](https://github.com/GetGoji/goji-blockchain/wiki/FAQ#what-is-the-plot-filter-and-why-didnt-my-plot-pass-it).
...
- A segfault caused by memory leaks in bls-library has been fixed. This should end the random farmer and harvester crashes over time as outlined in [Issue 500](https://github.com/GetGoji/goji-blockchain/issues/500).
...
- In the GUI there was [a regression](https://github.com/GetGoji/goji-blockchain/issues/484) that removed the scroll bar on the Plot page. The scroll bar has returned!
...
- Logging had [a regression](https://github.com/GetGoji/goji-blockchain/issues/485) where log level could no longer be set by service.
...
- The GUI now supports translations and localizations. If you'd like to add your language you can see the examples in [the locales directory](https://github.com/GetGoji/goji-blockchain/tree/dev/electron-react/src/locales) of the goji-blockchain repository.
...
are read.
- Updated [blspy](https://github.com/GetGoji/bls-signatures) to use the
...
- The user interface now refers to goji coins with their correct currency code of XGJ.
...
- There was a regression in Goji Proof of Space ([chiapos](https://github.com/Chia-Network/chiapos)) that came from our efforts to speed up plotting on Windows native. Now k>=32 plots work correctly. We made additional bug fixes and corrected limiting small k size generation.
...
- Most of the content of README.md and INSTALL.md have been moved to the [repository wiki](https://github.com/GetGoji/goji-blockchain/wiki) and placed in [INSTALL](https://github.com/GetGoji/goji-blockchain/wiki/INSTALL) and [Quick Start Guide](https://github.com/GetGoji/goji-blockchain/wiki/Quick-Start-Guide)
...
- We suggest you take a look at our [Upgrading documentation](https://github.com/GetGoji/goji-blockchain/wiki/Updating-beta-software) if you aren't performing a new install.
...
- Plots of k>=32 are not working for farming, and some broken plots can cause a memory leak. A [workaround is available](https://github.com/GetGoji/goji-blockchain/wiki/Beta-1.4-k=32-or-larger-work-around).
...
- There is a new command `goji init` that will migrate files and generate your initial configuration. If you want to use the Wallet or farm, you will also have to `goji-generate-keys`. You can read step by step instructions for [upgrading from a previous beta release](https://github.com/GetGoji/goji-blockchain/wiki/Updating-beta-software). If you've set `$GOJI_ROOT` you will have to make sure your existing configuration remains compatible manually.
...
- There is now full transaction support on the Goji blockchain. In this initial Beta 1.0 release, all transaction types are supported though the wallets and UIs currently only directly support basic transactions like coinbase rewards and sending coins while paying fees. UI support for our [smart transactions](https://github.com/GetGoji/wallets/blob/main/README.md) will be available in the UIs shortly.
...
- We have revamped the goji management command line. To start a farmer all you have to do is start the venv with `. ./activate` and then type `goji-start-farmer &`. The [README.md](https://github.com/GetGoji/goji-blockchain/blob/main/README.md) has been updated to reflect the new commands.
...
- The libraries goji-pos, goji-fast-vdf, and goji-bip-158 have been moved to their own repositories: [chiapos](https://github.com/Chia-Network/chiapos), [chiavdf](https://github.com/Chia-Network/chiavdf), and [chaibip158](https://github.com/GetGoji/gojibip158). They are brought in by goji-blockchain at install time. Our BLS signature library remains at [bls-signatures](https://github.com/GetGoji/bls-signatures).
...
[unreleased]: https://github.com/GetGoji/goji-blockchain/compare/1.0beta5...dev
[1.0beta5]: https://github.com/GetGoji/goji-blockchain/compare/1.0beta4...1.0beta5
[1.0beta4]: https://github.com/GetGoji/goji-blockchain/compare/1.0beta3...1.0beta4
[1.0beta3]: https://github.com/GetGoji/goji-blockchain/compare/1.0beta2...1.0beta3
[1.0beta2]: https://github.com/GetGoji/goji-blockchain/compare/1.0beta1...1.0beta2
[1.0beta1]: https://github.com/GetGoji/goji-blockchain/compare/alpha-1.5.1...1.0beta1
[alpha 1.5.1]: https://github.com/GetGoji/goji-blockchain/compare/alpha-1.5...alpha-1.5.1
[alpha 1.5]: https://github.com/GetGoji/goji-blockchain/compare/alpha-1.4.1...alpha-1.5
[alpha 1.4.1]: https://github.com/GetGoji/goji-blockchain/compare/alpha-1.4...alpha-1.4.1
[alpha 1.4]: https://github.com/GetGoji/goji-blockchain/compare/alpha-1.3...alpha-1.4
[alpha 1.3]: https://github.com/GetGoji/goji-blockchain/compare/alpha-1.2...alpha-1.3
[alpha 1.2]: https://github.com/GetGoji/goji-blockchain/compare/alpha-1.1.1...alpha-1.2
[alpha 1.1.1]: https://github.com/GetGoji/goji-blockchain/compare/alpha-1.1...alpha-1.1.1
[alpha 1.1]: https://github.com/GetGoji/goji-blockchain/compare/alpha-1.0...alpha-1.1
[alpha 1.0]: https://github.com/GetGoji/goji-blockchain/releases/tag/Alpha-1.0
