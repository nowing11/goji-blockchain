...
from goji.ssl.create_ssl import generate_ca_signed_cert, get_goji_ca_crt_key, make_ca_cert
...
def check_keys(new_root: Path) -> None:
    ...
    if "xgj_target_address" not in config["farmer"]:
        print(f"Setting the xgj destination address for coinbase fees reward to {all_targets[0]}")
    ...
    if "xgj_target_address" not in config["pool"]:
        print(f"Setting the xgj destination address for coinbase reward to {all_targets[0]}")
...
def create_all_ssl(root: Path):
    ...
    goji_ca_crt_path.write_bytes(goji_ca_crt)
    goji_ca_key_path.write_bytes(goji_ca_key)
